...
const TLS_CLIENT_PATH = path.join(__dirname, '../bin/tls-client-api-linux-amd64-1.11.0');
...
function tlsClientRequest({ url, method = 'GET', headers = {}, body = null, proxy = '' }) {
  return new Promise((resolve, reject) => {
    ...
    execFile(TLS_CLIENT_PATH, args, { timeout: 30000 }, (error, stdout, stderr) => {
      if (error) return reject(new Error(`TLS client failed: ${stderr || error.message}`));
      try {
