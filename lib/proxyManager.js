...
async function fetchGeoNodeProxies() {
  try {
    ...
    const geoProxies = res.data.data.map(p => ({
      ip: p.ip,
      port: 9000, // ✅ GeoNode’s default SOCKS5 port
...
async function getLockedProxy(userId) {
  ...
  // ✅ Validate proxy object before locking
  if (!proxy || !proxy.ip || !proxy.port || !proxy.username || !proxy.password) {
    console.error('❌ Invalid proxy object returned:', proxy);
    return null;
  }

  lockedProxies.set(userId, proxy);
  console.log(`🌍 Using Proxy for ${userId}: ${proxy.ip}:${proxy.port}:${proxy.username}:${proxy.password}`);
  return proxy;
}
