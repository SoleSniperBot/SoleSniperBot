...
const { generateRandomName, generateNikeEmail, generatePassword, generateRandomDOB } = require('./utils');
...
async function createNikeAccountWithBrowser(email, password, proxyString) {
  ...
  const { firstName, lastName } = generateRandomName(); // ✅ FIXED
  const dob = generateRandomDOB(); // ✅ FIXED
