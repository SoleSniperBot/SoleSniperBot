...
async function loginToNikeAndSaveSession(email, password, proxyString, maxAttempts = 3) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      // Try all possible login button selectors
      try {
        await Promise.race([
      ...
      // Wait for navigation or user icon
      try {
        await Promise.race([
        ...
        console.warn('⚠️ Login may have succeeded without full navigation');
      }
...
module.exports = { loginNikeAccount: loginToNikeAndSaveSession };
