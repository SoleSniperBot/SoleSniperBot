...
const tlsClientPath = path.join(__dirname, '../bin/tls-client');
...
async function loginWithBrowser(email, password, proxy = null) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    const formatted = proxyObj.formatted;
    ...
    try {
      ...
      if (res.body?.error === 'verification_required') {
        console.warn(`📨 2FA required for ${email}. Fetching via IMAP...`);
        const confirmed = await fetchNike2FACodeAndConfirm(email);
        if (confirmed) {
          console.log(`✅ 2FA confirmed for ${email}, retrying login...`);
          continue;
        } else {
          console.warn(`❌ 2FA failed or not received for ${email}`);
        }
      }
