...
const { markEmailUsed } = require('../lib/emailHelper');
...
async function createNikeAccount(email, password, proxy) {
  ...
  try {
    ...
    console.warn(`⚠️ Nike API failed (${err.response?.status || 'Unknown'}): ${err.message}`);
    
    const result = await fallbackBrowserSignup({ email, password, proxy });
    if (result.success) {
      markEmailUsed(email);
    }
    return result;
