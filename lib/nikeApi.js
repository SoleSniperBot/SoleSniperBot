...
async function createNikeAccount(email, password) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      console.log('â Nike account created:', res.data);
      return res.data;
    } catch (err) {
      lastError = err;
      console.warn(`â ï¸ Attempt ${attempt} failed:`, err.code || err.message);

      if (err.response && err.response.status >= 400 && err.response.status < 500) {
        console.error('â Stopped retry due to 4XX:', err.response.data);
        return null;
      }
  ...
  console.warn('â ï¸ Falling back to browser automation...');
